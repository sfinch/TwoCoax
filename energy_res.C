...
void energy_res(){
	...
	TFile *fin = new TFile(Form("1Msim.root"));
	TFile *fout = new TFile("out.root", "recreate");

	TTree *tin = (TTree *) fin->Get("tree2");
	TTree *tout = tin->CloneTree(0);
		
	for (Int_t i=0; i<2; i++){
		int det = i+1;

		tin->SetBranchAddress(Form("GE%iEN",det),&nt_Ge[i]);
		tout->Branch(Form("GE%iRes",det),&smear_Ge[i],Form("GE%iRes/D",det));

		hEnSmear[i] = new TH1F(Form("hEnSmear%i",i),Form("hEnSmear%i",i),6000,0,3000);
	}
	tin->SetBranchAddress("NaI",&nt_NaI);
